...
if _VERSION == "Lua 5.3" then
	...
	function packbytes(str)
		...
		size = math.floor(size / 256)
		local d = size
		return table.concat( {string.char(a),string.char(b),string.char(c),string.char(d) , str},"")
	end
	...
	function packvalue(id)
		...
		return table.concat( {string.char(a) , string.char(b)},"")
	end
end
...
function convert.protocol(all, obj)
	...
	for _, p in ipairs(obj[3]) do
		...
		if type(typename) == "table" then
			...
			typename = table.concat( {obj[1] , p[1]}, ".") 
...
function convert.type(all, obj)
	...
	for _, f in ipairs(obj[2]) do
		if f.type == "field" then
			...
			local nesttypename = table.concat({typename , f[1]},".")
			f[1] = nesttypename
...
local function checktype(types, ptype, t)
	...
	local fullname = table.concat({ptype , t},".")
