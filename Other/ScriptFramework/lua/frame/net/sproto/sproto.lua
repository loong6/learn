...
function host:dispatch(...)
	...
	if header.type then
		...
		if header_tmp.session then
			return "REQUEST", proto.name, proto.tag, result, gen_response(self, proto.response, header_tmp.session)
		else
			return "REQUEST", proto.name, proto.tag, result
		end
	else
		-- response
...
function host:attach(sp)
	return function(name, args, session)
		...
		if args then
			...
			return core.pack(table.concat( {header ,  content},""))
		else
			return core.pack(header)
		end
	end
